...
FROM	
        [sys].[database_scoped_credentials] [dbc] WITH (NOLOCK) ) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
TOP 1
...
WHERE [db].[name] = DB_NAME()) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
WHERE [dsc].[name] != 'MAXDOP' AND
      ...
      [dsc].[name] != 'QUERY_OPTIMIZER_HOTFIXES') AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
AND     [dp].[name] != N'cdc') AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  [dp].[principal_id]                   AS [ObjectId],
        [dp].[name]                           AS [Name],
...
) AS [_results] ORDER BY FilegroupId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
) AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
SELECT  [ssk].[name] AS [KeyName],
        ...
        (CASE 
            WHEN EXISTS (SELECT 1 FROM [sys].[key_encryptions] [ske] WITH (NOLOCK) WHERE [ske].[key_id] = [ssk].[symmetric_key_id] AND [ske].[crypt_type] = 'ESKP') 
                THEN 'ESKP' 
            WHEN EXISTS (SELECT 1 FROM [sys].[key_encryptions] [ske] WITH (NOLOCK) WHERE [ske].[key_id] = [ssk].[symmetric_key_id] AND [ske].[crypt_type] = 'ESP2') 
                THEN 'ESP2' 
            WHEN EXISTS (SELECT 1 FROM [sys].[key_encryptions] [ske] WITH (NOLOCK) WHERE [ske].[key_id] = [ssk].[symmetric_key_id] AND [ske].[crypt_type] = 'ESP3') 
                THEN 'ESP3' 			
            ELSE 
                NULL END)               AS [EncryptionType],
...
LEFT	JOIN [sys].[cryptographic_providers] [cpr] WITH (NOLOCK) ON [cpr].[guid] = [ssk].[cryptographic_provider_guid] ) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
AND SCHEMA_NAME([st].[schema_id]) <> N'cdc'
) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  [st].[user_type_id]             AS [TypeId],
        [st].[name]                     AS [Name],
...
) AS [_results] ORDER BY TableTypeId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  
...
WHERE   
        [t].[is_user_defined] = 1
) AS [_results] ORDER BY TableId,ColumnId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT DISTINCT
...
WHERE 
            ...
            AND SCHEMA_NAME([tt].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ColumnSourceId,ConstraintId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  
    SCHEMA_NAME([tt].[schema_id])  AS [SchemaName]
   ,[tt].[user_type_id]           AS [ColumnSourceId]
...
WHERE 
    ...
    AND [i].[is_unique_constraint] = 1
    AND	[o].[object_id] NOT IN (SELECT [major_id] FROM [sys].[extended_properties] WITH (NOLOCK) WHERE [minor_id] = 0 AND [class] = 1 AND [name] = N'microsoft_database_tools_support')
) AS [_results] ORDER BY ColumnSourceId,IndexId,KeyOrdinal  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT DISTINCT
    SCHEMA_NAME([tt].[schema_id]) AS [SchemaName]
   ,[tt].[user_type_id]        AS [ColumnSourceId]
...
WHERE [i].[is_primary_key] = 0
    ...
    AND [i].[is_hypothetical] = 0
    AND [i].[name] IS NOT NULL) AS [_results] ORDER BY ColumnSourceId,IndexId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  
    SCHEMA_NAME([tt].[schema_id]) AS [SchemaName]
   ,[tt].[user_type_id]        AS [ColumnSourceId]
...
WHERE [i].[is_primary_key] = 0
    AND [i].[is_unique_constraint] = 0
    AND [i].[name] IS NOT NULL
    AND [i].[is_hypothetical] = 0) AS [_results] ORDER BY ColumnSourceId,IndexId,KeyOrdinal  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        SCHEMA_NAME([t].[schema_id])        AS [SchemaName]
...
WHERE   [pf].[is_system] = 0) AS [_results] ORDER BY PartitionFunctionId,BoundaryId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
WHERE   [s].[is_system] IS NULL OR [s].[is_system] = 0) AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        [s].[data_space_id] AS [ObjectId],
        [s].[name]          AS [Name],
...
WHERE   ([s].[is_system] IS NULL OR [s].[is_system] = 0)
        AND ([d].[is_system] IS NULL OR [d].[is_system] = 0)) AS [_results] ORDER BY ObjectId,DestinationId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
       ...
       ,[sm].[is_inlineable] AS [IsInlineable]
       ,[sm].[inline_type] AS [IsInline]
...
WHERE      
           ...
           AND SCHEMA_NAME([o].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY FunctionId,ParameterOrder  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT    [o].[object_id]              AS [FunctionId]
          ,SCHEMA_NAME([o].[schema_id]) AS [FunctionOwner]
          ,[o].[name]                   AS [FunctionName]
          ...
          ,CAST(ISNULL([i].[last_value], 0) AS DECIMAL(38))
                                        AS [IdentityLastValue]
          ,NULL                         AS [IsOutput]
          ,NULL                         AS [IsReadOnly]
...
WHERE      
          ...
          AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       )) AND OBJECTPROPERTY([o].[object_id], N'IsEncrypted') = 0) AS [_results] ORDER BY FunctionId,ParameterOrder  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        [s].[schema_id]        AS [OwnerId]
...
WHERE
        ...
        AND SCHEMA_NAME([s].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY FunctionId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT    [o].[object_id]              AS [FunctionId]
          ,SCHEMA_NAME([o].[schema_id]) AS [FunctionOwner]
          ,[o].[name]                   AS [FunctionName]
...
WHERE      
          ...
          AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       )) AND ((OBJECTPROPERTY([o].[object_id], N'IsEncrypted') = 1) OR [o].[type] IN (N'FS', N'FT'))) AS [_results] ORDER BY FunctionId,ParameterOrder  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
AND SCHEMA_NAME([o].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY AggregateId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT     [o].[object_id]              AS [FunctionId]
          ,SCHEMA_NAME([o].[schema_id]) AS [FunctionOwner]
          ,[o].[name]                   AS [FunctionName]
...
AND SCHEMA_NAME([sp].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ProcedureId,ProcedureNumber,SequenceNumber DESC OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT 
...
WHERE    
    ...
    AND SCHEMA_NAME([p].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ProcedureId,ProcedureNumber,ParameterOrder  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        [sp].[schema_id]              AS [SchemaId]
       ,SCHEMA_NAME([sp].[schema_id]) AS [ProcedureOwner]
...
AND SCHEMA_NAME([sp].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ProcedureId,ProcedureNumber  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT 
     [p].[object_id]            AS [ProcedureId]
    ,SCHEMA_NAME([p].[schema_id]) 
...
) AS [_results] ORDER BY TableId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  [t].[name] AS [ColumnSourceName], 
        ...
        CAST(ISNULL([ic].[seed_value], 0) AS DECIMAL(38)) AS [IdentitySeed],
        CAST(ISNULL([ic].[increment_value], 0) AS DECIMAL(38)) AS [IdentityIncrement],
        CAST(ISNULL([ic].[last_value], 0) AS DECIMAL(38)) AS [IdentityLastValue],
        CONVERT(bit, [c].[is_nullable]) AS [IsNullable],
        [cc].[definition] AS [ComputedText],
        [c].[is_rowguidcol] AS [IsRowGuidColumn],
        [c].[collation_name] AS [Collation],
        ...
        [sc].[label_id] AS [SensitivityLabelId],
        [sc].[information_type] AS [SensitivityInformationType],
        [sc].[information_type_id] AS [SensitivityInformationTypeId],
        [sc].[rank] AS [SensitivityRank]
FROM    [sys].[columns] [c] WITH (NOLOCK)
...
LEFT JOIN [sys].[sensitivity_classifications] [sc] WITH (NOLOCK) ON [t].[object_id] = [sc].[major_id] and [c].[object_id] = [sc].[major_id] and [c].[column_id] = [sc].[minor_id]
...
AND SCHEMA_NAME([t].[schema_id]) <> N'cdc') AS [_results] ORDER BY TableId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  [t].[name] AS [ColumnSourceName], 
        [t].[object_id] AS [TableId],
        SCHEMA_NAME([t].[schema_id]) AS [SchemaName], 
...
WHERE   [ftc].[object_id] IS NULL AND ([fs].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [fs].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       ))) AS [_results] ORDER BY ColumnSourceId,ConstraintId,KeyOrdinal  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        [d].[object_id]                     AS [ConstraintId]
...
WHERE
        ...
        AND SCHEMA_NAME([d].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY TableId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
AND SCHEMA_NAME([sv].[schema_id]) <> N'cdc'
) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
AND SCHEMA_NAME([v].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ViewId,ColumnId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
        [sv].[schema_id]              AS [SchemaId]
       ,SCHEMA_NAME([sv].[schema_id]) AS [SchemaName]
...
WHERE ([sv].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [sv].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       )) AND OBJECTPROPERTY([sv].[object_id], N'IsEncrypted') = 1
AND SCHEMA_NAME([sv].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ViewId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
AND SCHEMA_NAME([v].[schema_id]) <> N'cdc') AS [_results] ORDER BY ViewId,ColumnId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT * FROM (
SELECT DISTINCT
    SCHEMA_NAME([o].[schema_id]) AS [SchemaName]
...
WHERE      [o].[type] IN (N'U', N'V')
           ...
           AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       ))) AS [_results] ORDER BY ColumnSourceId,StatisticsId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT     SCHEMA_NAME([o].[schema_id]) AS [SchemaName]
          ,[s].[object_id]              AS [ColumnSourceId]
          ,[o].[name]                   AS [ColumnSourceName]
...
WHERE      [o].[type] IN (N'U', N'V')
           ...
           AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       ))) AS [_results] ORDER BY ColumnSourceId,StatisticsId,StatsColumnId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
) AS [_results] ORDER BY StopListId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (SELECT 
 [spl].[name]              AS [Name],
 ...
 LEFT JOIN [sys].[database_principals] AS [p] WITH(NOLOCK) ON [spl].[principal_id] = [p].[principal_id]) AS [_results] ORDER BY PropertyListId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (SELECT 
...
INNER JOIN [sys].[registered_search_property_lists] AS [spl] WITH(NOLOCK) ON [spl].[property_list_id] = [sp].[property_list_id]) AS [_results] ORDER BY PropertyListId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
...
WHERE      [i].[is_hypothetical] = 0
           ...
           AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       ))) AS [_results] ORDER BY ColumnSourceId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT     SCHEMA_NAME([o].[schema_id]) AS [SchemaName]
...
WHERE 
    ...
    AND ([o].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
                                        ...
                                        WHERE     [major_id] = [o].[object_id]
                                              ...
                                              AND [class] = 1
                                              AND [name] = N'microsoft_database_tools_support'
                                       ))) AS [_results] ORDER BY XmlIndexType,ColumnSourceId,IndexId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT 
    SCHEMA_NAME([o].[schema_id]) AS [SchemaName]
    ,[i].[object_id]        AS [ColumnSourceId]
...
WHERE
           ...
           AND SCHEMA_NAME([o].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY ParentId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT DISTINCT
           [s].[name]      AS [SchemaName],
           [t].[parent_id] AS [ParentId],
...
) AS [_results] ORDER BY ObjectId DESC OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
) AS [_results] ORDER BY TriggerId DESC OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT DISTINCT
        [t].[object_id]         AS [ObjectId],
        [t].[name]              AS [Name],
...
SELECT
    SCHEMA_NAME([sec].[schema_id]) AS [SchemaName]
    ,[sec].[name] AS [EdgeConstraintName]
    ,[sec].[is_system_named] AS [IsSystemNamed]
    ,[sec].[object_id] AS [ConstraintId]
    ,[sec].[parent_object_id] AS [ParentTableId]
    ,OBJECT_NAME([sec].[parent_object_id]) AS [ParentTableName]
    ,SCHEMA_NAME([st].[schema_id]) AS [ParentTableSchemaName]
    ,[sec].[is_disabled] AS [IsDisabled]
    ,[sec].[is_not_trusted] AS [IsNotTrusted]
    ,[sec].[delete_referential_action] AS [DeleteAction]
FROM
    [sys].[edge_constraints] AS [sec]
    INNER JOIN [sys].[tables] AS [st] ON [st].[object_id] = [sec].[parent_object_id]
 OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));

SELECT
    [secc].[object_id] AS [ParentEdgeConstraintId]
    ,SCHEMA_NAME([st].[schema_id]) AS [ParentTableSchemaName]
    ,[sec].[name] AS [ParentEdgeConstraintName]
    ,[secc].[clause_number] AS [ClauseNumber]
    ,[secc].[from_object_id] AS [FromTableId]
    ,[secc].[to_object_id] AS [ToTableId]
FROM
    [sys].[edge_constraint_clauses] AS [secc]
    INNER JOIN [sys].[edge_constraints] AS [sec] ON [sec].[object_id] = [secc].[object_id]
    INNER JOIN [sys].[tables] AS [st] ON [st].[object_id] = [sec].[parent_object_id]
 OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
AND SCHEMA_NAME([ssy].[schema_id]) <> N'cdc'
) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
WHERE     OBJECTPROPERTY([so].[object_id], N'IsDefault') <> 0
          AND ([so].[is_ms_shipped] = 0 AND NOT EXISTS (SELECT *
            ...
            AND SCHEMA_NAME([so].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY LegacyId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
WHERE      OBJECTPROPERTY([do].[object_id], N'IsRule') <> 0
           ...
           AND SCHEMA_NAME([co].[schema_id]) <> N'cdc'
) AS [_results] ORDER BY LegacyId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT     [sc].[name]        AS [UddtSchemaName]
          ,[st].[name]        AS [UddtName]
...
) AS [_results] ORDER BY ContractId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
...
) AS [_results] ORDER BY ServiceId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
) AS [_results] ORDER BY EventNotificationId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
) AS [_results] ORDER BY AuditSpecificationId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
) AS [_results] ORDER BY ObjectName  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT [c].[class]                      AS [ObjectClass]
      ,[c].[major_id]                   AS [ObjectId]
      ,[c].[thumbprint]                 AS [Thumbprint]
...
) AS [_results] ORDER BY ObjectId,ChildObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (SELECT 
[s].[schema_id] AS [SchemaId],
...
LEFT JOIN [sys].[types] [types] WITH (NOLOCK) ON [s].[user_type_id] = [types].[user_type_id]) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (SELECT    
...
SELECT 
        ...
        LEFT JOIN [sys].[views] AS [v] WITH(NOLOCK) ON [pr].[target_object_id] = [v].[object_id] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
...
SELECT
        ...
        CAST([cmk].[allow_enclave_computations] AS bit) as [AllowEnclaveComputations],
...
SELECT
    ...
    [eds].[connection_options]                      AS [ConnectionOptions],
    [eds].[pushdown]                                AS [PushdownOption]
FROM
   ...
   LEFT  JOIN [sys].[database_credentials] AS [c]  WITH (NOLOCK) ON [c].[credential_id] = [eds].[credential_id]
    WHERE [eds].[type] <> 3) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
    [eff].[name]                             AS [Name],
...
FROM
    [sys].[external_file_formats]   AS [eff] WITH (NOLOCK)) AS [_results] OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  
        [t].[schema_id]                             AS [SchemaId],
        SCHEMA_NAME([t].[schema_id])                AS [SchemaName],
...
WHERE
 ...
 ORDER BY TypeId,GranteeId,GrantorId,MajorId,MinorId,PermissionCode,PermissionState  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT  [db].[class] AS [TypeId],
        [db].[major_id] AS [MajorId],
        [db].[minor_id] AS [MinorId],
...
WHERE
 ...
 ORDER BY TypeId,GranteeId,GrantorId,MajorId,PermissionStateOrder,MinorId,PermissionCode,PermissionState  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (SELECT
    CASE
    ...
    WHERE ([o].[ReferencingSchemaName] <> N'cdc' OR [o].[parent_class] <> 1) AND [o].[type] <> N'PG' AND [d].[referenced_database_name] IS NULL AND [d].[referenced_server_name] IS NULL AND [d].[is_ambiguous] = 0 AND  ([d].[is_caller_dependent] = 0 OR 1 = (SELECT COUNT(1) FROM [sys].[objects] WITH (NOLOCK) WHERE [name] = [d].[referenced_entity_name])) AND [c2].[graph_type] IS NULL AND (([o].[is_ms_shipped] = 0 OR [o].[type] = 'TT' OR [ParentTableType].[user_type_id] IS NOT NULL) AND [exp].[major_id] IS NULL)) AS [_results] ORDER BY ReferencingSchemaName,ReferencingName,ReferencingColumnName  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
...
WHERE
        [sp].[class] = 1
        AND [sp].[name] <> N'microsoft_database_tools_support' OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
...
SELECT
        ...
        CASE WHEN EXISTS(SELECT *  FROM [sys].[external_tables] WHERE [object_id] = [sp].[major_id])
            THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
        END AS [IsExternalTable]
FROM
...
WHERE
    ...
    AND (OBJECTPROPERTY([so].[parent_object_id], N'TableIsFake') = 0 OR OBJECTPROPERTY([so].[parent_object_id], N'TableIsFake')  IS NULL) OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
...
SELECT
    [sdp].[principal_id] AS [OwnerId],
    [sdp].[name] AS [LibraryOwner],
    [sel].[name] AS [LibraryName],
    [sel].[external_library_id]	AS [ObjectId],
    [sel].[language] AS [LibraryLanguage]
FROM
    [sys].[external_libraries] AS [sel] WITH (NOLOCK)
LEFT JOIN [sys].[database_principals] [sdp] WITH (NOLOCK) ON [sdp].[principal_id] = [sel].[principal_id]) AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
    [sdp].[principal_id] AS [OwnerId],
    [sdp].[name] AS [LibraryOwner],
    ...
    [sel].[external_library_id]	AS [ObjectId],
    [sel].[language] AS [LibraryLanguage],
    [self].[content] AS [LibraryContent],
    CAST([self].[platform] AS INT) AS [PlatformId],
    [self].[platform_desc] AS [Platform]
FROM
    [sys].[external_libraries] AS [sel] WITH (NOLOCK)
LEFT JOIN [sys].[database_principals] [sdp] WITH (NOLOCK) ON [sdp].[principal_id] = [sel].[principal_id]
LEFT JOIN [sys].[external_library_files] AS [self] WITH (NOLOCK) ON [sel].[external_library_id] = [self].[external_library_id]) AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
    [sdp].[principal_id] AS [OwnerId],
    [sdp].[name] AS [LanguageOwner],
    [sel].[language] AS [LanguageName],
    [sel].[external_language_id] AS [ObjectId]
FROM
    [sys].[external_languages] AS [sel] WITH (NOLOCK)
LEFT JOIN [sys].[database_principals] [sdp] WITH (NOLOCK) ON [sdp].[principal_id] = [sel].[principal_id]
where
    [sel].[language] != 'R' and [sel].[language] != 'Python') AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
SELECT * FROM (
SELECT
    [sdp].[principal_id] AS [OwnerId],
    [sdp].[name] AS [LanguageOwner],
    [sel].[external_language_id] AS [ObjectId],
    [sel].[language] AS [LanguageName],
    [self].[content] AS [LanguageContent],
    CAST([self].[platform] AS INT) AS [PlatformId],
    [self].[platform_desc] AS [Platform],
    [self].[parameters] as [Parameters],
    [self].[environment_variables] as [EnvironmentVariables],
    [self].[file_name] as [FileName]
FROM
    [sys].[external_languages] AS [sel] WITH (NOLOCK)
LEFT JOIN [sys].[database_principals] [sdp] WITH (NOLOCK) ON [sdp].[principal_id] = [sel].[principal_id]
LEFT JOIN [sys].[external_language_files] AS [self] WITH (NOLOCK) ON [sel].[external_language_id] = [self].[external_language_id]
where
    [sel].[language] != 'R' and [sel].[language] != 'Python') AS [_results] ORDER BY ObjectId  OPTION (USE HINT('FORCE_LEGACY_CARDINALITY_ESTIMATION'));
