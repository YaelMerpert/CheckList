...
SELECT @current_date = create_date FROM sys.databases WHERE database_id = 2
--SET @current_date = DATEADD(d,-35, GETDATE()) --testing
IF (DATEDIFF(d,@current_date, GETDATE()) > 1 AND UPPER(DB_NAME()) NOT IN ('MASTER','TEMPDB','MODEL','MSDB'))
BEGIN
...
,cte_InterestingIndexes2 AS (
       SELECT Distinct SCHEMA_NAME(o.schema_id) schema_name, o.schema_id, 
...
,cte_IndexUsage AS (
       ...
       JOIN sys.dm_db_index_usage_stats (NOLOCK) ius ON ii.object_id = ius.object_id AND ii.index_id = ius.index_id AND ius.database_id = DB_ID()
...
SELECT DISTINCT TOP 100 
       ...
       index_id,
       index_name, 
       SUM(index_size_kb) as index_size_kb,
       SUM(index_usage) as index_usage,
       SUM(index_updates) as index_updates,
       SUM(index_updates_for_sorting) as index_updates_for_sorting,
       MAX(last_index_usage_date) as last_index_usage_date,
       SUM(index_updates_for_sorting*index_size_kb) as Size
FROM cte_IndexUsage 
GROUP BY 
       schema_id,
       schema_name,
